...
const ohMy = () => {
    axios.get('http://localhost:3000/animals').then((res) => {
        ...
        for (let i = 0; i < res.data.length; i++){
            ...
            // let text = document.createTextNode(newElement.textContent = (res.data[i]))
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/LoveMusic').then((res) => {
...
/*
    ...
    select the button with the id "query-button" and add a click event listener that calls your function.
*/

// CODE HERE
function queryRequest() {
    axios.get('http://localhost:3000/query-test/?').then(({data}) => {
        console.log(data)
    
    // queryButton.style.display ='block';
     // queryButton.textContent = res.data;
...
/*
    ...
    1: Send no queries on the URL -- what happened? "You sent an empty query" in console.log

    2: Send more than 1 query on the URL -- what happened? "You sent more than 1 query!" in console.log
*/
